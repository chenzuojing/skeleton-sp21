...
public class Repo {
  ...
  /**
   * Lazy loading and caching: Let’s say you store the state of which files have been gitlet added
   * to your repo in your filesytem. Lazy loading: The first time you want that list of files when
   * you run your Java program, you need to load it from disk. Caching: The second time you need
   * that list of files in the same run of the Java program, don’t load it from disk again, but use
   * the same list as you loaded before. If you need to, you can then add multiple files to that
   * list object in your Java program. Writing back: When you Java program is finished, at the very
   * end, since you had loaded that list of files and may have modified it, write it back to your
   * file system.
   *
   * <p>Add a file to the staging area. If the current working version of the file is identical to
   * the version in the current commit, do not stage it to be added, and remove it from the staging
   * area if it is already there (as can happen when a file is changed, added, and then changed
   * back).
   */
  ...
  /**
   * Checks if the current working version of the file is identical to the version in the current
   * commit.
   */
  ...
  /**
   * Compares the byte array of the file in CWD and the byte array saved in the last commit/blob.
   *
  ...
  public void commit(String[] args) throws IOException {
    ...
    Branch currBranch = Utils.readObject((Utils.join(Main.GITLET_FOLDER, "HEAD")), Branch.class);
  ...
  /**
   * By default a commit is the same as its parent. Files staged for addition and removal are the
   * updates to the commit.
   */
  public Map<String, String> updateSnapshot() {
    ...
    stagedForRemovalFiles.forEach(parentSnapshot::remove);

    return parentSnapshot;
  }

  /**
   * Remove a file from the staging area (hashmap). Unstage the file if it is currently staged for
   * addition. If the file is tracked in the current commit, stage it for removal and remove the
   * file from the working directory if the user has not already done so (do not remove it unless it
  ...
  /**
   * Print the history of a commit tree. Starting at the current head commit, display information
  ...
  /**
   * Checks if a given commit id, full or abbreviated, matches the SHA1 id of a commit node
   *
  ...
  /**
   * Restore file from blob, put it in current working directory, and overwriting the version of the
  ...
  public String currentBranchName() {
    ...
    return Utils.readObject(branchFile, Branch.class).getName();
  }

  /**
   * TBD.
   *
   * <p>checkout should use abbreviated filename.
   */
  ...
  /**
   * Checks if a working file is untracked in the HEAD of current branch and would be overwritten by
   * the checkout.
   *
  ...
  /**
   * Find untracked files for reset 1. there exists some file (we’ll call it hi.txt) in the working
   * directory that is untracked in the most recent commit (a file called hi.txt does not exist in
   * the most recent commit’s snapshot) AND 2. The given commit (passed in as sha1) has a file
   * called hi.txt that is tracked (it exists in that commit’s snapshot) — for checkout, this
   * condition could be modified to be in reference to the latest commit on the given branch (rather
   * than a given commit ID)
   *
   * @param givenBranchHEAD name of the checkout branch
   */
  ...
  public String printBlob(String blobSHA) {
    ...
    String currContent = new String(blobObj.getFileContent(), StandardCharsets.UTF_8);
    return currContent;
  }
  ...
  private class Merge {
    ...
    public void merge(String branchName) throws IOException {
      ...
      String originalBranchName =
          Utils.readObject((Utils.join(Main.GITLET_FOLDER, "HEAD")), Branch.class).getName();
    ...
    public void restoreFilesAtMerge(
    ...
    public void condition3(
      ...
      sameOnCurrAndSP.forEach(
          (sameFileName, sameBlob) -> {
            diffOnGivenAndSP.forEach(
                (diffFileName, diffBlob) -> {
    ...
    public void condition4(
      ...
      sameOnGivenAndSP.forEach(
          (sameFileName, sameBlob) -> {
            diffOnCurrAndSP.forEach(
                (diffFileName, diffBlob) -> {
                  if (sameFileName.equals(diffFileName)) {
                    mergeMap.put(diffFileName, diffBlob);
                    stagingArea.add(diffFileName, diffBlob);
    ...
    public void condition5(
      ...
      for (String SPFileName : SP.keySet()) {
        ...
        if (insideCurr && insideGiven) {
          ...
          if (!givenBlob.equals(SPBlob) && !currBlob.equals(SPBlob) && givenBlob.equals(currBlob)) {
    ...
    public void condition6(
        Map<String, String> SP, Map<String, String> curr, Map<String, String> mergeMap) {
    ...
    public void condition7(
        Map<String, String> SP, Map<String, String> given, Map<String, String> mergeMap) {
    ...
    public void createConflictFile(String currBlob, String givenBlob) throws IOException {
      ...
      if (currBlobFile != null && givenBlobFile != null) {
        ...
        String currContent = new String(currBlobObj.getFileContent(), StandardCharsets.UTF_8);
        String givenContent = new String(givenBlobObj.getFileContent(), StandardCharsets.UTF_8);
    ...
    public void condition10_2And10_3(File presentBlobFile, String absentBranch) throws IOException {
      ...
      String presentContent = new String(presentBlobObj.getFileContent(), StandardCharsets.UTF_8);
    ...
    public void commitMerge(String branchName, String originalBranchName) throws IOException {
      ...
      Commit mergeCommit =
          new Commit(
              ...
              secondParentSHA1,
              false,
              stagingArea.getFilesStagedForAddition(),
    ...
    public void getAncestors(
        HashMap<Commit, Integer> ancestors,
